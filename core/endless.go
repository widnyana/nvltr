...
func (srv *EndlessServer) RegisterSignalHook(prePost int, sig os.Signal, f func()) (err error) {
	if prePost != PRESIGNAL && prePost != POSTSIGNAL {
		err = fmt.Errorf("Cannot use %v for prePost arg. Must be endless.PRESIGNAL or endless.POSTSIGNAL.", sig)
		return
	}
...
// ListenAndServeTLSfromString will serve with pre-loaded certificate
func ListenAndServeTLSfromString(addr string, certFile string, keyFile string, handler http.Handler) error {
	server := NewServer(addr, handler)
	return server.ListenAndServeTLSfromString(certFile, keyFile)
}

func (srv *EndlessServer) ListenAndServeTLSfromString(certFile, keyFile string) (err error) {
	addr := srv.Addr
	if addr == "" {
		addr = ":https"
	}
	...
	config.Certificates = make([]tls.Certificate, 1)
	config.Certificates[0], err = tls.X509KeyPair([]byte(certFile), []byte(keyFile))
	if err != nil {
		return
	}
	...
	LogAccess.Infof("Running at port: %s with PID: %d", srv.Addr, syscall.Getpid())
	return srv.Serve()
}
